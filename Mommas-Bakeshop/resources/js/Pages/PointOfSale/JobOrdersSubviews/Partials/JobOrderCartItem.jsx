...
import { Minus, Plus, Trash2, Edit2, Package, Wand2 } from "lucide-react";
import { cn } from "@/lib/utils";

const currency = (value) => `P${Number(value || 0).toFixed(2)}`;

export default function JobOrderCartItem({ item, type, onIncrement, onDecrement, onRemove, onEdit }) {
	const isCustom = type === "custom";
	
	return (
		<div className={cn(
			"group relative flex flex-col gap-3 p-3 rounded-lg border border-border/50 transition-all shadow-sm",
			isCustom ? "bg-primary-soft/30 border-primary/20" : "bg-card hover:border-primary/30"
		)}>
			<div className="flex justify-between items-start gap-2">
				<div className="min-w-0">
					<div className="flex items-center gap-1.5">
						{isCustom ? <Wand2 className="h-3 w-3 text-primary shrink-0" /> : <Package className="h-3 w-3 text-muted-foreground shrink-0" />}
						<h5 className="text-sm font-semibold truncate text-foreground group-hover:text-primary transition-colors">
							{isCustom ? item.description : item.ProductName}
						</h5>
					</div>
			...
			<div className="flex items-center justify-between mt-1">
				{!isCustom ? (
					...
					<div 
						className="flex items-center gap-2 px-2 py-1 bg-primary/10 rounded-md border border-primary/20 text-xs font-bold text-primary cursor-pointer hover:bg-primary/20"
						onClick={onEdit}
					>
						Qty: {item.quantity}
					</div>
