...
export default function StockIn({ stockIns, onEdit, onHeaderMetaChange }) {
	...
	const totalPages = Math.max(1, Math.ceil(filteredStockIns.length / itemsPerPage));
	const safeCurrentPage = Math.min(currentPage, totalPages);
	const startIndex = (safeCurrentPage - 1) * itemsPerPage;
	const paginatedStockIns = filteredStockIns.slice(startIndex, startIndex + itemsPerPage);
				...
				<div className="flex flex-1 min-w-0 items-center gap-2">
					<div className="relative flex-1 min-w-0">
						<div className="overflow-x-auto pb-1 pr-4">
							...
							<select value={supplierFilter} onChange={(e) => setSupplierFilter(e.target.value)} className="w-40 rounded-md border-gray-300 text-sm focus:border-primary focus:ring-primary">
							...
							<select value={hasPurchaseDateFilter} onChange={(e) => setHasPurchaseDateFilter(e.target.value)} className="w-44 rounded-md border-gray-300 text-sm focus:border-primary focus:ring-primary">
							...
							<input type="number" min="0" step="0.01" placeholder="Min Amount" value={minTotalAmount} onChange={(e) => setMinTotalAmount(e.target.value)} className="w-32 rounded-md border-gray-300 text-sm focus:border-primary focus:ring-primary" />
							<input type="number" min="0" step="0.01" placeholder="Max Amount" value={maxTotalAmount} onChange={(e) => setMaxTotalAmount(e.target.value)} className="w-32 rounded-md border-gray-300 text-sm focus:border-primary focus:ring-primary" />
							</div>
						</div>
				...
				<div className="sticky bottom-0 z-10 border-t border-gray-200 bg-white px-4 py-3 sm:px-6">
					<div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
						<div className="text-sm text-gray-600">
							Showing {filteredStockIns.length === 0 ? 0 : startIndex + 1}-{Math.min(startIndex + itemsPerPage, filteredStockIns.length)} of {filteredStockIns.length}
						</div>
						<div className="flex flex-wrap items-center gap-2">
							<label htmlFor="stock-in-items-per-page" className="text-sm text-gray-600">Items per page</label>
							<select
								id="stock-in-items-per-page"
								value={itemsPerPage}
