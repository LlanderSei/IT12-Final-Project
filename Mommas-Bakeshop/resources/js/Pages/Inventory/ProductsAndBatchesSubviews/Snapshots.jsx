...
export default function Snapshots({ snapshots = [], onHeaderMetaChange }) {
	...
	const filteredSnapshots = useMemo(() => {
		...
		return [...filtered].sort((a, b) => {
			const getValue = (record) => {
				...
				return record.SnapshotTime
					? new Date(record.SnapshotTime).getTime()
					: 0;
			};
	...
	return (
		<div className="bg-white p-6 flex-1 flex flex-col overflow-hidden min-h-0">
				...
				<div className="mb-6 flex items-start gap-3">
					<div className="relative w-full max-w-xl shrink-0">
						...
						<input
							...
							placeholder="Search by snapshot creator, date, or product..."
							value={searchQuery}
					...
					<div className="sticky bottom-0 z-10 border-t border-gray-200 bg-white px-4 py-3 sm:px-6">
						<div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
							<div className="text-sm text-gray-600">
								...
								{Math.min(startIndex + itemsPerPage, filteredSnapshots.length)}{" "}
								of {filteredSnapshots.length}
							</div>
							<div className="flex flex-wrap items-center gap-2">
								<label
									htmlFor="product-snapshot-items-per-page"
									className="text-sm text-gray-600"
								>
									Items per page
								</label>
								<select
									id="product-snapshot-items-per-page"
									value={itemsPerPage}
					...
					<div className="p-6">
						...
						<div className="text-sm text-gray-600 space-y-1 mb-4">
							...
							<p>
								Total Amount:{" "}
								<span className="font-medium text-gray-800">
									P{Number(selectedSnapshot?.TotalAmount || 0).toFixed(2)}
								</span>
							</p>
							<p>
								Snapshot Time:{" "}
								...
								<tbody className="bg-white divide-y divide-gray-200">
									{(selectedSnapshot?.Leftovers || []).map((line) => (
										<tr key={line.ID}>
											...
											<td className="px-4 py-2 text-sm text-gray-700">
												P{Number(line.PerUnitAmount || 0).toFixed(2)}
											</td>
											<td className="px-4 py-2 text-sm text-gray-900">
												{line.LeftoverQuantity}
											</td>
											<td className="px-4 py-2 text-sm text-gray-900">
												P{Number(line.LineAmount || 0).toFixed(2)}
											</td>
										</tr>
									))}
