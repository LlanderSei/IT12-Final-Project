...
export default function ProductionBatches({
	...
	const [itemFilter, setItemFilter] = useState(filters.item || "all");
	...
	const [currentPage, setCurrentPage] = useState(
		Number(filters.page || batches.current_page || 1),
	);
	const [itemsPerPage, setItemsPerPage] = useState(
		Number(filters.perPage || batches.per_page || 25),
	);
	...
	const producedProducts = useMemo(
		() =>
			(products || []).filter(
				(product) =>
					String(product.ProductFrom || "").toLowerCase() === "produced",
			),
		[products],
	);
	...
	const records = batches.data || [];
	const addedByOptions = filterOptions.addedBy || [];
	const itemNameOptions = filterOptions.item || [];

	useEffect(() => {
		setSearchQuery(filters.search || "");
		setAddedByFilter(filters.addedBy || "all");
		setItemFilter(filters.item || "all");
		...
		setCurrentPage(Number(filters.page || batches.current_page || 1));
		setItemsPerPage(Number(filters.perPage || batches.per_page || 25));
	}, [
		filters.search,
		filters.addedBy,
		filters.item,
	...
	useEffect(() => {
		...
		const timeoutId = window.setTimeout(() => {
			router.get(
				fetchRoute,
				{
					batchSearch: searchQuery,
					batchAddedBy: addedByFilter,
					batchItem: itemFilter,
					batchDateFrom: dateFrom,
					batchDateTo: dateTo,
					batchMinTotalQty: minTotalQty,
					batchMaxTotalQty: maxTotalQty,
					batchSortKey: sortConfig.key,
					batchSortDirection: sortConfig.direction,
					batchPerPage: itemsPerPage,
					page: currentPage,
				},
				{
					preserveState: true,
					preserveScroll: true,
					replace: true,
					only: ["batches", "batchFilters", "batchFilterOptions"],
				},
			);
		}, 250);
	...
	const totalPages = Math.max(1, Number(batches.last_page || 1));
	...
	const countLabel = formatCountLabel(Number(batches.total || 0), "record");
	const visibleRangeLabel = useMemo(() => {
		const from = Number(batches.from || 0);
		const to = Number(batches.to || 0);
		const total = Number(batches.total || 0);
		return `Showing ${from}-${to} of ${total}`;
	}, [batches.from, batches.to, batches.total]);
	...
	const openCreateModal = () => {
		if (!canCreateProductionBatch) {
			return requirePermission("CanCreateProductionBatch");
		}
		setIsModalOpen(true);
	};
	...
	const openEditBatchDraft = (batch) => {
		if (!canUpdateProductionBatch) {
			return requirePermission("CanUpdateProductionBatch");
		}
	...
	const addExistingProduct = (product) => {
		...
		const issues = [];
		if (!String(row.QuantityProduced ?? "").trim()) {
			issues.push(`"${product.ProductName}": Quantity is required.`);
		}
		if (qty <= 0) issues.push(`"${product.ProductName}": Quantity must be greater than 0.`);
		if (!Number.isInteger(qty)) {
			issues.push(`"${product.ProductName}": Quantity must be a whole number.`);
		}
		if (issues.length) {
	...
	const addNewProduct = () => {
		const newProduct = draft.newProduct;
		const qty = Number(newProduct.QuantityProduced || 0);
		const price = Number(newProduct.Price || 0);
		const issues = [];
		if (!String(newProduct.ProductName || "").trim()) {
			issues.push("New Product: Product Name is required.");
		}
		if (!String(newProduct.CategoryID || "").trim()) {
			issues.push("New Product: Category is required.");
		}
		if (!String(newProduct.Price || "").trim()) {
			issues.push("New Product: Price is required.");
		}
		if (!String(newProduct.QuantityProduced || "").trim()) {
			issues.push("New Product: Quantity is required.");
		}
		if (qty <= 0) issues.push("New Product: Quantity must be greater than 0.");
		if (!Number.isInteger(qty)) {
			issues.push("New Product: Quantity must be a whole number.");
		}
		...
		setDraft((prev) => ({
			...
			items: [
				...prev.items,
				{
					...
					ItemName: newProduct.ProductName,
					QuantityProduced: qty,
					CreateProduct: {
	...
	const saveEditLine = () => {
		...
		if (!Number.isInteger(qty)) {
			issues.push("Edit line: Quantity must be a whole number.");
		}
									...
									<div className="relative flex-1 min-w-0">
										<div className="overflow-x-auto pb-1 pr-4">
											<div className="flex min-w-max items-center gap-2 pr-3">
												...
												<select value={itemFilter} onChange={(e) => { setItemFilter(e.target.value); setCurrentPage(1); }} className="w-44 rounded-md border-gray-300 text-sm focus:border-primary focus:ring-primary">
										...
										<thead className="bg-gray-50 sticky top-0 z-10 shadow-sm">
											<tr>
												<th className="w-28 px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" onClick={() => requestSort("AddedBy")}>
													Added By {sortConfig.key === "AddedBy" && <span className="ml-1 text-[10px] text-gray-400">{sortConfig.direction.toUpperCase()}</span>}
												</th>
												<th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Items Produced</th>
												<th className="w-24 px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" onClick={() => requestSort("TotalQuantity")}>
													Total Quantity {sortConfig.key === "TotalQuantity" && <span className="ml-1 text-[10px] text-gray-400">{sortConfig.direction.toUpperCase()}</span>}
												</th>
												<th className="w-52 px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" onClick={() => requestSort("BatchDescription")}>
													Batch Description {sortConfig.key === "BatchDescription" && <span className="ml-1 text-[10px] text-gray-400">{sortConfig.direction.toUpperCase()}</span>}
												</th>
												<th className="w-32 px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" onClick={() => requestSort("DateAdded")}>
													Batch Date {sortConfig.key === "DateAdded" && <span className="ml-1 text-[10px] text-gray-400">{sortConfig.direction.toUpperCase()}</span>}
												</th>
												...
												<tr key={batch.ID} className="hover:bg-gray-50 align-top">
													...
													<td className="px-4 py-4 text-right">
														{canUpdateProductionBatch ? (
															<button type="button" onClick={() => openEditBatchDraft(batch)} className="inline-flex items-center gap-1.5 rounded bg-primary px-3 py-1 text-xs font-medium text-white hover:bg-primary-hover">
								...
								<div className="sticky bottom-0 z-10 border-t border-gray-200 bg-white px-4 py-3 sm:px-6">
									<div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
										...
										<div className="flex flex-wrap items-center gap-2">
											...
											<select id="production-batches-items-per-page" value={itemsPerPage} onChange={(e) => { setItemsPerPage(Number(e.target.value)); setCurrentPage(1); }} className="rounded-md border-gray-300 text-sm focus:border-primary focus:ring-primary">
											...
											{pageNumbers.map((page) => (
												<button key={page} type="button" onClick={() => goToPage(page)} className={`rounded-md px-3 py-1.5 text-sm ${page === safeCurrentPage ? "border border-primary bg-primary text-white" : "border border-gray-300 text-gray-700 hover:bg-gray-50"}`}>
													{page}
												</button>
											))}
							...
							<div className="relative w-full max-w-md rounded-lg bg-white p-5 shadow-xl">
								...
								<div className="space-y-2 text-sm">
									{editingLine.CreateProduct && (
										<>
											<input className="w-full rounded border-gray-300" value={editingLine.ItemName} onChange={(e) => setEditingLine((prev) => ({ ...prev, ItemName: e.target.value }))} placeholder="Product Name" />
											<select className="w-full rounded border-gray-300" value={editingLine.CreateProduct.CategoryID || ""} onChange={(e) => setEditingLine((prev) => ({ ...prev, CreateProduct: { ...prev.CreateProduct, CategoryID: e.target.value } }))}>
												<option value="">Select Category</option>
												{(categories || []).map((cat) => (
													<option key={cat.ID} value={cat.ID}>{cat.CategoryName}</option>
												))}
											</select>
											<input type="number" min="0" step="0.01" className="w-full rounded border-gray-300" value={editingLine.CreateProduct.Price || ""} onChange={(e) => setEditingLine((prev) => ({ ...prev, CreateProduct: { ...prev.CreateProduct, Price: e.target.value } }))} placeholder="Price" />
										</>
									)}
									<input type="number" min="1" className="w-full rounded border-gray-300" value={editingLine.QuantityProduced} onChange={(e) => setEditingLine((prev) => ({ ...prev, QuantityProduced: e.target.value }))} placeholder="Quantity" />
								</div>
