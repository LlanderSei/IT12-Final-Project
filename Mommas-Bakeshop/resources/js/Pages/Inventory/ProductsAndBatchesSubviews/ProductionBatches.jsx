		...
		<div className="flex flex-col flex-1 w-full relative overflow-hidden min-h-0">
						...
						<div className="p-6 flex-1 flex flex-col overflow-hidden min-h-0">
							...
							<div className="mb-6 flex items-start gap-3">
								...
								<div className="flex flex-1 min-w-0 items-center gap-2">
									<div className="relative flex-1 min-w-0">
										<div className="overflow-x-auto pb-1 pr-4">
											<div className="flex min-w-max items-center gap-2 pr-3">
												...
												<select value={itemFilter} onChange={(e) => setItemFilter(e.target.value)} className="w-44 rounded-md border-gray-300 text-sm focus:border-[#D97736] focus:ring-[#D97736]">
									...
									<button type="button" onClick={resetFilters} className="shrink-0 rounded-md border border-gray-300 bg-white px-3 py-2 text-xs font-medium text-gray-700 shadow-sm hover:bg-gray-50">
										Reset Filters
									</button>
								</div>
							</div>
			...
			<div className="sticky bottom-0 w-full p-4 bg-white border-t border-gray-200 z-10">
				<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
					<button onClick={openModal} className="w-full flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-[#D97736] hover:bg-[#c2682e] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#D97736]">
						Record a Batch
					</button>
				</div>
			</div>
					...
					<div className="flex min-h-screen items-center justify-center p-4">
						...
						<form onSubmit={submitBatch} className="relative w-full max-w-7xl rounded-lg bg-white shadow-xl">
									...
									{draft.mode === "existing" ? (
											...
											{filteredProducedProducts.map((product) => (
												<div key={product.ID} className="rounded border p-2 text-sm">
													...
													<div className="flex items-center gap-2">
														...
														<button type="button" onClick={() => addExistingProduct(product)} className="shrink-0 rounded bg-[#D97736] px-3 py-2 text-xs text-white">Add</button>
													</div>
												</div>
											))}
										...
										<div className="space-y-2 text-sm rounded border p-3">
											...
											<div className="grid grid-cols-2 gap-2">
												...
												<button type="button" onClick={addNewProduct} className="rounded bg-[#D97736] px-3 py-2 text-xs text-white">Add New Product</button>
											</div>
										</div>
									)}
								</div>
							...
							<div className="flex justify-end gap-2 border-t bg-gray-50 px-6 py-3">
								...
								<button type="button" onClick={saveAndClose} className="rounded border border-[#D97736] px-4 py-2 text-sm text-[#D97736]">Save and Close</button>
								<button type="submit" disabled={batchForm.processing || (draft.items || []).length === 0} className="rounded bg-[#D97736] px-4 py-2 text-sm text-white disabled:opacity-50">Record Batch</button>
							</div>
						</form>
					</div>
						...
						<div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
							...
							<div className="relative w-full max-w-md rounded-lg bg-white p-5 shadow-xl">
								...
								<div className="mt-4 flex flex-wrap justify-end gap-2">
									...
									<button type="button" onClick={() => { setShowExitWarning(false); saveAndClose(); }} className="rounded border border-[#D97736] px-3 py-2 text-sm text-[#D97736]">Save and Exit</button>
