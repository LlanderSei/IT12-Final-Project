...
class DatabaseConnectionManager {
	...
	private function buildSchemaCompatibilityReport(ConnectionInterface $localConnection, ConnectionInterface $remoteConnection): array {
		$localTables = $this->schemaReportTables($localConnection);
		$remoteTables = $this->schemaReportTables($remoteConnection);
	...
	private function schemaReportTables(ConnectionInterface $connection): array {
		$excluded = config('database-connections.schema_excluded_tables', []);

		return array_values(array_filter(
			$this->schemaTables($connection),
			fn($table) => !in_array($table, $excluded, true),
		));
	}
